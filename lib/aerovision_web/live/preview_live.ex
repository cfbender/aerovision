defmodule AeroVisionWeb.PreviewLive do
  ...
  def mount(_params, _session, socket) do
    on_device? = Application.get_env(:aerovision, :target, :host) != :host

    socket =
      if on_device? do
        assign(socket, on_device?: true, pixels: nil)
      else
    ...
    {:ok, assign(socket, page_title: "Display Preview")}
  end
    ...
    <Layouts.app flash={@flash}>
      <%= if @on_device? do %>
        <div class="flex flex-col items-center justify-center min-h-[400px] text-center px-8">
          ...
          <h2 class="text-xl font-semibold text-white mb-2">Preview unavailable</h2>
          <p class="text-gray-400 max-w-md">
            The LED preview is only available in development mode.
            On this device, the physical LED panel shows the current display output.
          </p>
        </div>
      <% else %>
        <div class="space-y-4">
