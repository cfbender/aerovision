defmodule AeroVision.Flight.Utils.GeoTest do
  ...
  describe "bounding_box/3" do
    test "radius 0 produces a degenerate box where min == max for each axis" do
      {min_lat, min_lon, max_lat, max_lon} = Geo.bounding_box(35.0, -78.0, 0)
    ...
    test "positive radius produces max_lat > min_lat and max_lon > min_lon" do
      {min_lat, min_lon, max_lat, max_lon} = Geo.bounding_box(35.0, -78.0, 50)
    ...
    test "lat span is approximately 2 * radius / 111 degrees" do
      radius_km = 111.0
      {min_lat, _min_lon, max_lat, _max_lon} = Geo.bounding_box(35.0, -78.0, radius_km)
    ...
    test "center point is inside the returned box" do
      lat = 35.0
      lon = -78.0
      {min_lat, min_lon, max_lat, max_lon} = Geo.bounding_box(lat, lon, 50)
  ...
  describe "meters_to_feet/1" do
    test "nil returns nil" do
      assert Geo.meters_to_feet(nil) == nil
    end

    test "0 returns 0.0" do
      assert Geo.meters_to_feet(0) == 0.0
    end

    test "1.0 meter ≈ 3.28084 feet" do
      assert_in_delta Geo.meters_to_feet(1.0), 3.28084, 0.001
    end

    test "10000 meters ≈ 32808.4 feet" do
      assert_in_delta Geo.meters_to_feet(10_000), 32_808.4, 1.0
    end
  end
  ...
  describe "ms_to_knots/1" do
    test "nil returns nil" do
      assert Geo.ms_to_knots(nil) == nil
    end

    test "0 returns 0.0" do
      assert Geo.ms_to_knots(0) == 0.0
    end

    test "1.0 m/s ≈ 1.94384 knots" do
      assert_in_delta Geo.ms_to_knots(1.0), 1.94384, 0.001
    end

    test "100.0 m/s ≈ 194.384 knots" do
      assert_in_delta Geo.ms_to_knots(100.0), 194.384, 0.1
    end
  end
  ...
  describe "flight_progress/6" do
    ...
    test "at geographic midpoint returns approximately 0.5" do
      ...
      result =
        Geo.flight_progress(@origin_lat, @origin_lon, @dest_lat, @dest_lon, mid_lat, mid_lon)
    ...
    test "nil current coordinates returns nil" do
      assert Geo.flight_progress(@origin_lat, @origin_lon, @dest_lat, @dest_lon, nil, nil) ==
               nil
    end

    test "nil origin coordinates returns nil" do
      assert Geo.flight_progress(
               nil,
               @origin_lon,
               @dest_lat,
               @dest_lon,
